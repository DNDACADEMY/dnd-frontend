...
export const EventResultPanel = ({ eventName, eventId }: EventResultPanelProps) => {
  const [result, setResult] = useState<{ status: EventResultStatus; userName: string } | null>(null)
  ...
  const hasEvent = result != null
  const showFirecracker = result?.status === 'PASSED'
